---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-spell
tags:
- compendium/src/pf2e/crb
- spell/level/2
- trait/illusion
- trait/visual
aliases: ["Blur"]
---
# Blur *Spell 2*   
[illusion](archive/srd/pf2e/rules/traits/illusion.md "Illusion School Trait")  [visual](archive/srd/pf2e/rules/traits/visual.md "Visual Effect Trait")  

- **Traditions** [arcane](archive/srd/pf2e/rules/traits/arcane.md "Arcane Tradition Trait"), [occult](archive/srd/pf2e/rules/traits/occult.md "Occult Tradition Trait")
- **Cast** [>>](chapter-9-playing-the-game.md#Actions "Two-Action") 
- **Range**: touch
- **Targets**: 1 creature
- **Duration**: 1 minute

The target's form appears blurry. It becomes [concealed](archive/srd/pf2e/rules/conditions.md#Concealed). As the nature of this effect still leaves the target's location obvious, the target can't use this concealment to [Hide](archive/srd/pf2e/rules/actions/hide.md) or [Sneak](archive/srd/pf2e/rules/actions/sneak.md).

*Source: Core Rulebook p. 321*