...
> - **Success** The target is [stunned](archive/srd/pf2e/rules/conditions.md#Stunned) as it wrestles with the urge.
> - **Failure** On its next turn, before it does anything else, the target must present the nearest creature with an item in its possession; the target chooses which item to give, and if the only item it has is one that it's currently using to defend itself, such as a weapon during a combat encounter, it can choose to be [stunned](archive/srd/pf2e/rules/conditions.md#Stunned) for 1 round instead of giving up the item. This might require the target to [Interact](archive/srd/pf2e/rules/actions/interact.md) to retrieve an item or move to reach the nearest creature, and handing the item to the target always requires an [Interact](archive/srd/pf2e/rules/actions/interact.md) action.
