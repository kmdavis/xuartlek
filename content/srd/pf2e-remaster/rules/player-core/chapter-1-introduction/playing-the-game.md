...
In a Pathfinder game, three [[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/rules-overview#Modes of Play|modes of play]] determine the pacing of each scene in the story. Most of your character's time is spent in **[[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/exploration-mode|exploration]]**, uncovering mysteries, solving problems, and interacting with other characters. [[srd/pf2e-remaster/rules/player-core/chapter-1-introduction/golarion-and-the-inner-sea#The Age of Lost Omens|The Age of Lost Omens]] abounds with danger, however, and characters often find themselves in an **[[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/encounter-mode|encounter]]**, fighting savage beasts and terrifying monsters. Finally, time moves quickly when the characters enjoy **[[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/downtime-mode|downtime]]**, a respite from the world's troubles and a chance to rest and train for future expeditions. Throughout an adventure, gameplay moves between these three modes many times, as needed for the story. The more you play the game, the more you'll see that each mode has its own play style, but moving from mode to mode has few hard boundaries.

During the game, your character will face situations where the outcome is uncertain. A character might need to climb a sheer cliff, track down a wounded chimera, or sneak past a sleeping dragon, all of which are dangerous tasks with a price for failure. In such cases, the acting character (or characters) will be asked to attempt a [[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/checks|check]] to determine whether or not they succeed. A check is usually made by rolling a single 20-sided die (a d20) and adding a number based on the relevant attribute. In such cases, rolling high is always good.
...
PCs don’t have to spend every waking moment adventuring. They might also recover from wounds, plan future conquests, or pursue a trade. In Pathfinder, this is called [[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/downtime-mode|downtime]], and it allows time to pass quickly while characters work on long-term tasks or objectives. Most characters can practice a trade in downtime, earning a few coins, but those with the right skills can also spend time crafting, creating new gear or even magic items. Characters can also use downtime to retrain, replacing one character choice with another to reflect their evolving priorities. They might also research a problem, learn new spells, or even run a business or kingdom!
...
During encounters, each creature gets three [[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/actions|actions]] during their turn. These actions are spent to attack, interact with objects, move, and use special abilities. Actions available to all characters can be found in [[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/actions#Basic Actions|Chapter 8]].

### Ancestry

An ancestry is the broad family of people that a character belongs to. Ancestry determines a character's starting [[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/hit-points-healing-and-dying#Hit Points|Hit Points]], [[srd/pf2e-remaster/rules/player-core/chapter-2-ancestries-backgrounds/languages|languages]], [[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/perception-and-detection#Senses|senses]], and [[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/movement#Speed|Speed]], and it grants access to ancestry feats. Ancestries can be found in [[srd/pf2e-remaster/rules/player-core/chapter-2-ancestries-backgrounds|Chapter 2]].

### Armor Class (AC)

All creatures in the game have an [[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/checks#Armor Class|Armor Class]]. This score represents how hard it is to hit and damage a creature. It serves as the Difficulty Class for hitting a creature with an attack.

### Attack

When a creature tries to harm another creature, it makes a Strike or uses some other attack action. Most attacks are Strikes made with a weapon, but a character might Strike with their fist, grapple or shove with their hands, or attack with a [[srd/pf2e-remaster/rules/player-core/chapter-7-spells|spell]].

### Attribute Modifier

Each creature has six attribute modifiers: Strength, Dexterity, Constitution, Intelligence, Wisdom, and Charisma. Each of these numbers represents a creature's raw potential and general training. Attributes are described in full [[srd/pf2e-remaster/rules/player-core/chapter-1-introduction/character-creation|later in this chapter]].

### Background

A background represents what a character experienced before they took up the life of an adventurer. Each background grants a feat and training in one or more skills. You can read more about backgrounds in [[srd/pf2e-remaster/rules/player-core/chapter-2-ancestries-backgrounds/backgrounds|Chapter 2]].

### Bonuses And Penalties

[[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/checks#Bonuses|Bonuses]] and [[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/checks#Penalties|penalties]] apply to checks and certain statistics. There are several types of bonuses and penalties. If you have more than one bonus of the same type, you use only the highest bonus. Likewise, you use only the worst penalty of each type.

### Check

When a character attempts an action where success is uncertain, they may roll a [[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/checks|check]]. This is a roll of one twenty-sided die plus the named proficiency bonus. For example, a [[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/checks#Perception|Perception]] check would add your Perception proficiency.

### Class

A class represents the adventuring profession chosen by a character. A character's class determines most of their proficiencies, grants the character Hit Points each time they gain a new level, and gives access to a set of class feats. Classes appear in [[srd/pf2e-remaster/rules/player-core/chapter-3-classes|Chapter 3]].

### Condition

An ongoing effect that changes how a character can act, or that alters some of their statistics, is called a condition. The rules for the basic conditions used in the game can be found in the [[srd/pf2e-remaster/rules/player-core/conditions-appendix|Conditions Appendix]] at the back of this book.
...
The Game Master is the player who adjudicates the rules and narrates the various elements of the Pathfinder story and world that the other players explore. The GM uses the rules found in [[srd/pf2e-remaster/rules/gm-core/introduction|Pathfinder GM Core]].

### Golarion

Pathfinder is set on the planet Golarion during the Age of Lost Omens. It is an ancient world with a rich diversity of people and cultures, exciting locations to explore, and deadly villains. More information on the Age of Lost Omens, the world of Golarion, and its deities can be found on [[srd/pf2e-remaster/rules/player-core/chapter-1-introduction/golarion-and-the-inner-sea|here]].

### Hit Points (HP)

[[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/hit-points-healing-and-dying#Hit Points|Hit Points]] represent the amount of punishment a creature can take before it falls unconscious and begins dying. Damage decreases Hit Points on a 1-to-1 basis, while healing restores Hit Points at the same rate.

### Initiative

At the start of an encounter, all creatures involved roll for initiative to determine the order in which they act. The higher the result of its roll, the earlier a creature gets to act. Initiative and combat are described in [[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/encounter-mode|Chapter 8]].
...
Perception measures your character's ability to notice hidden objects or unusual situations, and it usually determines how quickly the character springs into action in combat. It is described in full in [[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/checks#Perception|Chapter 8]].
...
A round is a period of time during an [[srd/pf2e-remaster/rules/player-core/chapter-8-playing-the-game/encounter-mode|encounter]] in which all participants get a chance to act. A round represents approximately 6 seconds in game time.
...
Spells are magical effects created by performing mystical incantations and gestures known only to those with special training or inborn abilities. Casting a spell is an activity that usually uses 2 actions. Each spell specifies what it targets, the actions needed to cast it, its effects, and how it can be resisted. If a class grants spells, the basics of that ability are provided in the class description in [[srd/pf2e-remaster/rules/player-core/chapter-3-classes|Chapter 3]], while the spells themselves are detailed in [[srd/pf2e-remaster/rules/player-core/chapter-7-spells|Chapter 7]].
