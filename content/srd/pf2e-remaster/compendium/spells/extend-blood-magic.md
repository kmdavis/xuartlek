---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-spell
tags:
- compendium/src/pf2e-remaster/player-core-2
- trait/concentrate
- trait/focus
- trait/sorcerer
- trait/spellshape
- trait/uncommon
- trait/metamagic
- spell/level/3
aliases: ["Extend Blood Magic"]
aon_id: "spell-2115"
source: "Player Core 2"
---

# Extend Blood Magic  *Focus 3*  [>](rules/player-core/chapter-8-playing-the-game/actions#Actions "Single Action")

`Concentrate`  `Focus`  `Sorcerer`  `Spellshape`  `Uncommon`  `Metamagic`

- **Bloodline**: Imperial

You call upon the arcane energy coursing through your blood to extend the magic it grants. If your next action is to Cast a Spell that grants you a blood magic effect that lasts for at least 1 round, you or a target gain the blood magic effect for an additional round. You can have only one extended blood magic effect at a time.

*Source: Player Core 2 p. 265*
