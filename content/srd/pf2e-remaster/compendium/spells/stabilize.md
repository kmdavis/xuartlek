---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-spell
tags:
- compendium/src/pf2e-remaster/player-core
- trait/cantrip
- trait/concentrate
- trait/healing
- trait/manipulate
- trait/vitality
- trait/positive
- spell/level/1
aliases: ["Stabilize"]
aon_id: "spell-1689"
source: "Player Core"
---

# Stabilize  *Cantrip 1*  [>>](rules/player-core/chapter-8-playing-the-game/actions#Actions "Two-Action")

`Cantrip`  `Concentrate`  `Healing`  `Manipulate`  `Vitality`  `Positive`

- **Traditions**: Divine, Primal
- **Range**: 30 feet
- **Target**: 1 dying creature

Life energy shuts death's door. The target loses the dying condition, though it remains unconscious at 0 Hit Points.

*Source: Player Core p. 359*
