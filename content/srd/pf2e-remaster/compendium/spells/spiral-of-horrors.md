---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-spell
tags:
- compendium/src/pf2e-remaster/player-core
- trait/aura
- trait/concentrate
- trait/emotion
- trait/fear
- trait/focus
- trait/manipulate
- trait/mental
- trait/uncommon
- trait/wizard
- spell/level/4
aliases: ["Spiral of Horrors"]
aon_id: "spell-1899"
source: "Player Core"
---

# Spiral of Horrors  *Focus 4*  [>>](rules/player-core/chapter-8-playing-the-game/actions#Actions "Two-Action")

`Aura`  `Concentrate`  `Emotion`  `Fear`  `Focus`  `Manipulate`  `Mental`  `Uncommon`  `Wizard`

- **Area**: 30-foot emanation
- **Duration**: sustained up to 1 minute

Shades and spirits howl and whirl around you in a display that strikes fear into the hearts of all who witness it. Enemies in the area are frightened 1 and can't reduce their frightened value below 1 for the spell's duration.

*Source: Player Core p. 388*
