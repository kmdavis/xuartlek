---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-spell
tags:
- compendium/src/pf2e-remaster/player-core
- trait/concentrate
- trait/focus
- trait/incapacitation
- trait/mental
- trait/uncommon
- trait/wizard
- spell/level/1
aliases: ["Charming Push"]
aon_id: "spell-1902"
source: "Player Core"
---

# Charming Push  *Focus 1*  [>](rules/player-core/chapter-8-playing-the-game/actions#Actions "Single Action")

`Concentrate`  `Focus`  `Incapacitation`  `Mental`  `Uncommon`  `Wizard`

- **Range**: 30 feet
- **Target**: 1 creature
- **Defense**: Will
- **Duration**: until the start of your next turn

You push at the target's mind to deflect their ire. The target must attempt a Will save.

**Critical Success** The target is unaffected.

**Success** The target takes a –1 circumstance penalty to attack rolls and damage rolls against you.

**Failure** The target can't use hostile actions against you.

**Critical Failure** The target is stunned 1 and can't use hostile actions against you.

*Source: Player Core p. 388*
