---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-weapon
tags:
- compendium/src/pf2e-remaster/treasure-vault-remastered
- trait/deadly
- trait/disarm
- trait/thrown
- trait/uncommon
- weapon/level/0
aliases: ["Mambele"]
aon_id: "weapon-295"
source: "Treasure Vault (Remastered)"
---

# Mambele  *Weapon 0*

`Deadly`  `Disarm`  `Thrown`  `Uncommon`

- **PFS Note**: For the purposes of Pathfinder Society play, choosing the following region as a Home Region (or with the World Traveler boon) grants access this item: Mwangi Expanse
- **Price**: 6 sp
- **Damage**: 1d6 S
- **Bulk**: 1
- **Hands**: 1
- **Type**: Melee
- **Category**: Martial
- **Group**: Axe

Also known as a hunga munga or danisco, this knife-axe hybrid consists of a hilt and a blade that curves backward toward the wielder. The curve of the blade is such that after a victim has been struck by a mambele, more damage is dealt as the weapon is extracted from the victim's body.

### Critical Specialization Effects

Certain feats, class features, weapon runes, and other effects can grant you additional benefits when you make a Strike with certain weapons and get a critical success. This is called a critical specialization effect. The exact effect depends on which weapon group your weapon belongs to, as listed below. You can always decide not to add the critical specialization effect of your weapon.

*Source: Treasure Vault (Remastered) p. 27*
