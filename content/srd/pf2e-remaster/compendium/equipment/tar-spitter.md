---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-siege-weapon
tags:
- compendium/src/pf2e-remaster/battlecry
- trait/alchemical
- trait/mounted
- trait/uncommon
- siege-weapon/level/11
aliases: ["Tar Spitter"]
aon_id: "siege-weapon-64"
source: "Battlecry!"
---

# Tar Spitter  *Siege Weapon 11*

`Alchemical`  `Mounted`  `Uncommon`

- **Price**: 2,800 gp
- **Ammunition**: barrel of alchemical tar (5 gp, 5 Bulk)

**Usage** mounted

**Space** 16 feet long, 12 feet wide, 16 feet high

**Crew** 3 to 4

**Proficiency** simple

---

**AC** 24

**Fort** +20

**Ref** +14

**Hardness** 8

**HP** 120

**Immunities** object immunities 

---

**Speed** 10 feet (pulled or pushed)

---

A tar spitter is a mass of tubes, pumps, and a copper tank mounted on an armored wagon. The tank is filled with hot tar under high pressure that can be sprayed from an aimed nozzle. The weaponized adhesive often roots enemies to the spot, making them easier prey for allied soldiers. Cleaning a tar spitter is a difficult task, and crew members usually wear thick leather aprons and gloves to avoid getting the tar on their skin.
**Aim** [>>](rules/player-core/chapter-8-playing-the-game/actions#Actions "Two-Action") rotate 45° 
**Load** [>>](rules/player-core/chapter-8-playing-the-game/actions#Actions "Two-Action") (manipulate) 3 times, requires a successful DC 20 Athletics check 
**Launch** [>](rules/player-core/chapter-8-playing-the-game/actions#Actions "Single Action") (manipulate) 4d8 bludgeoning plus 4d8 acid and 1d8 persistent acid, 60-foot line or 30-foot cone, DC 27 Reflex. A creature on the ground who fails its save is also immobilized by the tar (Escape DC 27). Switching between the tar spitter’s line or cone mode takes an Interact action, and any of the crew can perform the action.

*Source: Battlecry! p. 106*
