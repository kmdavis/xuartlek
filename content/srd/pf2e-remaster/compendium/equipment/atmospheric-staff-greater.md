---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-equipment
tags:
- compendium/src/pf2e-remaster/rage-of-elements
- trait/air
- trait/magical
- trait/staff
- trait/uncommon
- equipment/level/12
aliases: ["Atmospheric Staff (Greater)"]
aon_id: "equipment-2576-2329"
source: "Rage of Elements"
---

# Atmospheric Staff (Greater)  *Item 4+*

`Air`  `Magical`  `Staff`  `Uncommon`

- **Usage**: held in 1 hand
- **Bulk**: 2

This staff is made of a dense wood and strikes the ground with an imposing boom. At the top of the staff is a perfectly round obsidian sphere that, when stared at for too long, makes viewers feel as though they're heavier than before. When wielding this staff, you gain a +1 item bonus to saves against forced movement.

**Activate** Cast a Spell; **Effect** You expend a number of charges from the staff to cast a spell from its list.

**Craft Requirements** Supply one casting of all listed ranks of all listed spells.

### Atmospheric Staff (Lesser)

**Price** 95 gp

**Bulk** 2

---

**Cantrip** _gale blast_**1st** _air bubble_, _gravitational pull_

### Atmospheric Staff

**Price** 480 gp

**Bulk** 2

---

**2nd** _gentle breeze_**3rd** _gravity well_, _tempest cloak_

### Atmospheric Staff (Greater)

**Price** 1,900 gp

**Bulk** 2

---

**4th** _fly_, _variable gravity_**5th** _elemental form_ (air only), _telekinetic haul_

### Atmospheric Staff (Major)

**Price** 8,900 gp

**Bulk** 2

---

**6th** _zero gravity_**7th** _elemental form_ (air only), _vacuum_

*Source: Rage of Elements p. 74*
