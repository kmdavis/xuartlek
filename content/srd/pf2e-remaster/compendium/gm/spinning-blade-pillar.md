---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-hazard
tags:
- compendium/src/pf2e-remaster/gm-core
- trait/complex
- trait/mechanical
- trait/trap
- hazard/level/4
aliases: ["Spinning Blade Pillar"]
aon_id: "hazard-494"
source: "GM Core"
---

# Spinning Blade Pillar  *Hazard 4*

`Complex`  `Mechanical`  `Trap`

- **Complexity**: Complex A metal pole with three razor-sharp spinning blades is hidden in the floor, connected to trigger plates in up to eight floor tiles and a hidden control panel within 30 feet.

**Stealth**
+11 (trained) or DC 26 (expert) to notice the control panel

**Disable**
DC 21 Thievery (trained) twice on the pillar, or DC 19 Thievery (expert) once on the control panel deactivates the whole trap. Breaking the control panel prevents anyone from disabling the trap using the control panel and prevents the trap from deactivating automatically (see Reset below).

**AC** 21, **Fort** +10, **Ref** +12

**Pillar Hardness**
12

**Pillar HP**
48 (BT 24); **Panel Hardness** 5; **Panel HP** 20 (BT 10)

**Immunities**
critical hits, object immunities, precision damage

**Rising Pillar** [](rules/player-core/chapter-8-playing-the-game/actions#Actions "Single Action") spinning blade +12, **Damage** 2d10+5 slashing

---
**Reset**
The trap deactivates and resets after 1 minute.

*Source: GM Core p. 107*
