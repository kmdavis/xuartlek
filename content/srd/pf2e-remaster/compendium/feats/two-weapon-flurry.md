---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-feat
tags:
- compendium/src/pf2e-remaster/player-core
- trait/fighter
- trait/flourish
- trait/press
- feat/level/14
aliases: ["Two-Weapon Flurry"]
aon_id: "feat-4846"
source: "Player Core"
---

# Two-Weapon Flurry  *Feat 14*  [>](rules/player-core/chapter-8-playing-the-game/actions#Actions "Single Action")

`Fighter`  `Flourish`  `Press`

- **Archetypes**: Dual-Weapon Warrior (Level 16), Dual-Weapon Warrior (Level 16)
- **Requirements**: You are wielding two weapons, each in a different hand

Using the momentum of your prior attack, you lash out with both your weapons in a sudden frenzy. Strike twice, once with each weapon.

*Source: Player Core p. 150*
