---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-feat
tags:
- compendium/src/pf2e-remaster/player-core
- trait/cleric
- trait/concentrate
- trait/spellshape
- trait/metamagic
- feat/level/6
aliases: ["Cast Down"]
aon_id: "feat-4661"
source: "Player Core"
---

# Cast Down  *Feat 6*  [>](rules/player-core/chapter-8-playing-the-game/actions#Actions "Single Action")

`Cleric`  `Concentrate`  `Spellshape`  `Metamagic`

The sheer force of your faith can bring a foe crashing down. If the next action you use is to cast _harm_ or _heal_ to damage one creature, the target is knocked prone if it takes any damage from the spell. If the target critically fails its save against the spell, it also takes a –10-foot status penalty to its Speed for 1 minute.

*Source: Player Core p. 116*
