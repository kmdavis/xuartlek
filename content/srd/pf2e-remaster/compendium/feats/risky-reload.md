---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-feat
tags:
- compendium/src/pf2e-remaster/guns-gears-remastered
- trait/flourish
- trait/gunslinger
- feat/level/2
aliases: ["Risky Reload"]
aon_id: "feat-3163"
source: "Guns & Gears (Remastered)"
---

# Risky Reload  *Feat 2*  [>](rules/player-core/chapter-8-playing-the-game/actions#Actions "Single Action")

`Flourish`  `Gunslinger`

- **Archetypes**: Unexpected Sharpshooter (Level 4), Unexpected Sharpshooter (Level 4)
- **Requirements**: You're wielding a firearm.

You've practiced a technique for rapidly reloading your firearm, but attempting to use this technique is a dangerous gamble with your firearm's functionality. Interact to reload a firearm, then make a Strike with that firearm. If the Strike fails, the firearm misfires.

*Source: Guns & Gears (Remastered) p. 112*
