---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-feat
tags:
- compendium/src/pf2e-remaster/player-core
- trait/fighter
- feat/level/8
aliases: ["Dueling Riposte"]
aon_id: "feat-4811"
source: "Player Core"
---

# Dueling Riposte  *Feat 8*  [<](rules/player-core/chapter-8-playing-the-game/actions#Actions "Reaction")

`Fighter`

- **Archetypes**: Duelist (Level 10), Duelist (Level 10)
- **Prerequisites**: Dueling Parry (Fighter)
- **Trigger**: A creature within your reach critically fails a Strike against you
- **Requirements**: You are benefiting from Dueling Parry

You elegantly counterattack against your flailing enemy. Make a melee Strike against or attempt to Disarm the triggering creature.

### Dueling Riposte leads to...

Guiding Riposte, Improved Dueling Riposte

*Source: Player Core p. 146*
