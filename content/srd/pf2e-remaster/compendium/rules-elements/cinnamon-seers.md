---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-item-bonus
tags:
- compendium/src/pf2e-remaster/treasure-vault-remastered
- trait/alchemical
- trait/consumable
- trait/lozenge
- item-bonus/level/4
aliases: ["Cinnamon Seers"]
aon_id: "equipment-1910-bonus-532"
source: "Treasure Vault (Remastered)"
---

# Cinnamon Seers  *Item 4*

`Alchemical`  `Consumable`  `Lozenge`

- **Price**: 14 gp
- **Usage**: held in 1 hand

Zippy, alchemically treated cinnamon suffuses cinnamon seers, a rock candy with a lively taste that provides a mental boost. A cinnamon seer remains in your mouth for 1 hour, its stimulating flavor granting you a +1 item bonus to checks to Recall Knowledge.

 **Secondary Effect** [<](rules/player-core/chapter-8-playing-the-game/actions#Actions "Reaction") (fortune) **Trigger** You gain no information from a Recall Knowledge check; **Effect** Reroll the triggering check. If this was a secret check, the GM rerolls rather than you; the candy doesn't give you any insight into what the GM rolled, so in that case, you're rerolling based only on guesswork. The seer becomes inert. You become temporarily immune to cinnamon seers until the next time you make your daily preparations.

*Source: Treasure Vault (Remastered) p. 47*
