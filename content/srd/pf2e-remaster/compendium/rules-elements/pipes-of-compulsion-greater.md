---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-item-bonus
tags:
- compendium/src/pf2e-remaster/treasure-vault-remastered
- trait/coda
- trait/occult
- trait/staff
- trait/uncommon
- item-bonus/level/8
aliases: ["Pipes of Compulsion (Greater)"]
aon_id: "equipment-2269-2035-bonus-696"
source: "Treasure Vault (Remastered)"
---

# Pipes of Compulsion (Greater)  *Item 4+*

`Coda`  `Occult`  `Staff`  `Uncommon`

- **Usage**: held in 2 hands
- **Bulk**: L

These panpipes are made of what seems to be beat-up tin bound by frayed leather and look like they shouldn't function at all, but in skilled hands they emit a beautiful sound that beguiles the senses. While playing the pipes, you gain a +1 item bonus to Diplomacy and Performance checks.

**Activate** Cast a Spell; **Effect** You expend a number of charges from this instrument to cast a spell from its list.

**Craft Requirements** Supply one casting of all listed levels of all listed spells.

### Pipes of Compulsion

**Price** 90 gp

**Bulk** L

---

**Cantrip** _daze_**1st** _charm_, _command_, _fear_

### Pipes of Compulsion (Greater)

**Price** 460 gp

**Bulk** L

---

**2nd** _laughing fit_, _stupefy_**3rd** _charm_, _hypnotize_, _mind reading_, _ring of truth_

### Pipes of Compulsion (Major)

**Price** 1,900 gp

**Bulk** L

---

The item bonuses are +2.
**4th** _confusion_, _suggestion_**5th** _charm_, _glimmer of charm_, _suggestion_, _telepathy_

*Source: Treasure Vault (Remastered) p. 137*
