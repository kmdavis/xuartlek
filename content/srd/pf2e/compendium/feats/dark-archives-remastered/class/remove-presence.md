---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-feat
tags:
- compendium/src/pf2e/dark-archives-remastered
- trait/amp
- trait/illusion
- trait/mental
- trait/occult
- trait/psychic
- feat/level/8
aliases: ["Remove Presence"]
aon_id: "feat-8329"
source: "Dark Archives (Remastered)"
---

# Remove Presence  *Feat 8*

`Amp`  `Illusion`  `Mental`  `Occult`  `Psychic`

Your spell reaches into the mind of a creature and removes you from it. The amped cantrip must be one that has one or more targets and must either require a spell attack roll or have a saving throw. Use this amp in place of the psi cantrip's normal amp entry.

---
**Amp** Choose one target of the spell or one creature in its area. If that enemy fails its save or the spell hits it, you become undetected by that creature, disappearing from its senses for 1 minute or until you use a hostile action. This is similar to being [[srd/pf2e/compendium/rules-elements/conditions#Invisible|invisible]], but effects like [[srd/pf2e/compendium/spells/rank-2/see-the-unseen|_see the unseen_]] don't reveal you—you're affecting the target's mind, not its vision. [[srd/pf2e/compendium/spells/rank-6/truesight|_Truesight_]] can still see through this illusion if the counteract check succeeds.

*Source: Dark Archives (Remastered) p. 27*
