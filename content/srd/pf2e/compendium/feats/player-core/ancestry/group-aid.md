---
cssclasses: pf2e,pf2e-feat
tags:
- compendium/src/pf2e/player-core
- trait/human
- feat/level/9
aliases: ["Group Aid"]
aon_url: "https://2e.aonprd.com/Feats.aspx?ID=4485"
source: "Player Core"
socialImage: og-image.png
---

# Group Aid  *Feat 9*

`Human`

Your upbringing emphasized teamwork, and helping your allies comes naturally to you. After you Aid an ally at a skill check that doesn’t have the attack trait, you can also Aid any other ally who attempts the same skill check for the same purpose that round. You do so as a free action rather than a reaction.

The preparation you did to help must still apply to the other allies, and you can Aid each ally only once. For example, if you helped lift up an ally to Aid them on an [[srd/pf2e/compendium/rules-elements/skills/athletics|Athletics]] check to scale a wall, you could keep the same posture to give a boost to other allies attempting to scale the wall in the same round.

*Source: Player Core p. 64*
