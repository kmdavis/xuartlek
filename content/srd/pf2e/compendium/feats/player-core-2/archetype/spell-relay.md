---
cssclasses: pf2e,pf2e-feat
tags:
- compendium/src/pf2e/player-core-2
- trait/concentrate
- trait/sorcerer
- feat/level/6
aliases: ["Spell Relay"]
aon_url: "https://2e.aonprd.com/Feats.aspx?ID=6103"
source: "Player Core 2"
---

# Spell Relay  *Feat 6*  [<](books/player-core/chapter-8-playing-the-game/actions#Actions "Reaction")

`Concentrate`  `Sorcerer`

- **Archetype**: Wellspring Mage (Level 8)
- **Trigger**: An ally [[srd/pf2e/compendium/rules-elements/actions/player-core#Cast a Spell|Casts a Spell]] and you are within that spell’s range.

You open the power in your blood to your ally's spellcasting, using your magic to boost their range. Your ally can use you as the point of origin for their spell, calculating range and cover from your space instead of their own.

*Source: Player Core 2 p. 154*
