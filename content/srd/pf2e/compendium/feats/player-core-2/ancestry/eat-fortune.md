---
cssclasses: pf2e,pf2e-feat
tags:
- compendium/src/pf2e/player-core-2
- trait/concentrate
- trait/divine
- trait/tengu
- feat/level/5
aliases: ["Eat Fortune"]
aon_url: "https://2e.aonprd.com/Feats.aspx?ID=5681"
source: "Player Core 2"
socialImage: og-image.png
---

# Eat Fortune  *Feat 5*  [<](books/player-core/chapter-8-playing-the-game/actions#Actions "Reaction")

`Concentrate`  `Divine`  `Tengu`

- **Frequency**: once per day
- **Trigger**: A creature within 60 feet uses a [[srd/pf2e/compendium/rules-elements/traits/player-core/fortune|fortune]] or [[srd/pf2e/compendium/rules-elements/traits/player-core/misfortune|misfortune]] effect.

As someone tries to twist fate, you consume the interference. The triggering effect is disrupted. If it's a misfortune effect, Eat Fortune gains the fortune trait; if it's a fortune effect, Eat Fortune gains the misfortune trait. This fortune or misfortune applies to the same roll the triggering effect would have, so you couldn't negate a fortune effect with Eat Fortune and then apply a misfortune effect to the same roll.

### Eat Fortune leads to...

[[srd/pf2e/compendium/feats/player-core-2/ancestry/jinx-glutton|Jinx Glutton]]

*Source: Player Core 2 p. 34*
