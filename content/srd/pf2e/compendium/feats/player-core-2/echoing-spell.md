---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-feat
tags:
- compendium/src/pf2e/player-core-2
- trait/concentrate
- trait/sorcerer
- trait/spellshape
- trait/metamagic
- feat/level/18
aliases: ["Echoing Spell"]
aon_id: "feat-6121"
source: "Player Core 2"
---

# Echoing Spell  *Feat 18*  [>](books/player-core/chapter-8-playing-the-game/actions#Actions "Single Action")

`Concentrate`  `Sorcerer`  `Spellshape`  `Metamagic`

- **Archetypes**: Wellspring Mage (Level 18), Time Mage (Level 20), Time Mage (Level 20)

You time the components of your spell with exacting precision, setting up a resonance that duplicates the spell's effects. If your next action is to Cast a Spell of 4th rank or lower that has no duration, the spell's energy reverberates and echoes. You can Cast the Spell a second time before the end of your next turn without expending a spell slot.

*Source: Player Core 2 p. 157*
