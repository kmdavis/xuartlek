---
cssclasses: pf2e,pf2e-feat
tags:
- compendium/src/pf2e/war-of-immortals
- trait/auditory
- trait/concentrate
- trait/exemplar
- feat/level/12
aliases: ["Extract Vow of Nonviolence"]
aon_url: "https://2e.aonprd.com/Feats.aspx?ID=7177"
source: "War of Immortals"
socialImage: og-image.png
---

# Extract Vow of Nonviolence  *Feat 12*  [>>](books/player-core/chapter-8-playing-the-game/actions#Actions "Two-Action")

`Auditory`  `Concentrate`  `Exemplar`

- **PFS Note**: When using Exact Vow of Nonviolence on an unattended item, the DC is a standard DC for the item's level.

You entice or threaten a weapon, making it swear not to harm you. Attempt a [[srd/pf2e/compendium/rules-elements/skills/Diplomacy|Diplomacy]] check to [[srd/pf2e/compendium/rules-elements/actions/player-core#Request|Request]] or an [[srd/pf2e/compendium/rules-elements/actions/player-core#Request|Intimidation]] check to [[srd/pf2e/compendium/rules-elements/actions/player-core#Demoralize|Demoralize]] against one weapon within 60 feet—even though most weapons are inanimate and mindless, your words somehow still can affect it as if it were living and you shared a language. The DC for this check is the Will DC of the creature wielding the weapon or a moderate DC for the weapon's level if it's unattended. If the weapon is an intelligent item, use its Will DC. Regardless of the result of the save, the weapon is then temporarily immune to Extract Vow of Nonviolence for 1 day.

**Special** When you gain this feat, choose cold iron, duskwood, dawnsilver, or silver. For some reason—perhaps because you underestimate this material, or because you think it a friend—you are forever unable to use Extract Vow of Nonviolence on objects composed of the chosen material.

**Critical Success** The object swears not to hurt you. You gain [[srd/pf2e/books/player-core/chapter-8-playing-the-game/Immunity, Weakness, and Resistance#Resistance|resistance]] to all damage dealt by that specific weapon as the weapon attempts to mitigate harm to you. The resistance is equal to half your level and lasts for 10 minutes.

**Success** As critical success, but the remaining duration decreases by 1 minute each time the weapon hits you (even if it deals no damage).

*Source: War of Immortals p. 39*
