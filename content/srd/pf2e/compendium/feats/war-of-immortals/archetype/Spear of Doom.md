---
cssclasses: pf2e,pf2e-feat
tags:
- compendium/src/pf2e/war-of-immortals
- trait/archetype
- trait/stance
- feat/level/10
aliases: ["Spear of Doom"]
aon_url: "https://2e.aonprd.com/Feats.aspx?ID=7264"
source: "War of Immortals"
socialImage: og-image.png
---

# Spear of Doom  *Feat 10*  [>](books/player-core/chapter-8-playing-the-game/actions#Actions "Single Action")

`Archetype`  `Stance`

- **Archetype**: [[srd/pf2e/compendium/character/archetypes/Warrior Of Legend|Warrior Of Legend]]
- **Prerequisites**: [[srd/pf2e/compendium/feats/war-of-immortals/archetype/Warrior Of Legend Dedication|Warrior Of Legend Dedication]]
- **Requirements**: An enemy hit you with an attack dealing the same damage type as your cursed weakness within the last round, and you are [[srd/pf2e/compendium/rules-elements/Conditions#Doomed|doomed]] 1 or greater.

The conditions for your doom have been met, and you brace yourself for a battle that will see you either live to fight another day or die a legend. You enter the spear of doom stance. While in this stance, as long as the enemy who triggered your cursed weakness is within your reach, you can Strike them as a reaction whenever they succeed or critically succeed at an attack against you. You gain an additional reaction immediately and at the start of each of your subsequent turns that can only be used to make [[srd/pf2e/compendium/feats/player-core-2/archetype/Reactive Strike|Reactive Strikes]] against that opponent, or to make the [[srd/pf2e/compendium/rules-elements/actions/player-core#Strike|Strike]] granted by this stance.

While in spear of doom stance, your cursed weakness is equal to your level instead of half your level.

### Spear of Doom leads to...

[[srd/pf2e/compendium/feats/war-of-immortals/archetype/Razor's Edge|Razor's Edge]]

*Source: War of Immortals p. 67*
