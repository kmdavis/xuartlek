---
cssclasses: pf2e,pf2e-feat
tags:
- compendium/src/pf2e/rage-of-elements
- trait/archetype
- trait/metal
- trait/spellshape
- trait/metamagic
- feat/level/10
aliases: ["Reverberating Spell"]
aon_url: "https://2e.aonprd.com/Feats.aspx?ID=4347"
source: "Rage of Elements"
socialImage: og-image.png
---

# Reverberating Spell  *Feat 10*  [>](books/player-core/chapter-8-playing-the-game/actions#Actions "Single Action")

`Archetype`  `Metal`  `Spellshape`  `Metamagic`

- **Archetype**: [[srd/pf2e/compendium/character/archetypes/elementalist|Elementalist]]
- **Prerequisites**: [[srd/pf2e/compendium/feats/rage-of-elements/archetype/elementalist-dedication|Elementalist Dedication]]; metal is in your elemental philosophy

Your spell disorients your targets with a metallic clangor. If the next action you use this turn is to Cast a non-cantrip Spell that deals damage in an area (such as a burst, line, or cone), the spell deals an additional 1d8 sonic damage and all creatures who fail their save against the spell are [[srd/pf2e/compendium/rules-elements/conditions#Deafened|deafened]] for 1 round. Targets who critically fail their saves against this spell are instead deafened for 1 minute. The spell gains the [[srd/pf2e/compendium/rules-elements/traits/player-core/sonic|sonic]] trait.

*Source: Rage of Elements p. 59*
