---
cssclasses: pf2e,pf2e-feat
tags:
- compendium/src/pf2e/rage-of-elements
- trait/electricity
- trait/impulse
- trait/kineticist
- trait/manipulate
- trait/metal
- trait/primal
- feat/level/8
aliases: ["Conductive Sphere"]
aon_url: "https://2e.aonprd.com/Feats.aspx?ID=4259"
source: "Rage of Elements"
socialImage: og-image.png
---

# Conductive Sphere  *Feat 8*  [>>](books/player-core/chapter-8-playing-the-game/actions#Actions "Two-Action")

`Electricity`  `Impulse`  `Kineticist`  `Manipulate`  `Metal`  `Primal`

A floating metal ball forms in a space within 30 feet, flashing with electricity. It can't be targeted or damaged. Any of your allies adjacent to it gain resistance to electricity equal to your level and add the _shock_ rune to all their Strikes with metal objects. The sphere lasts until the end of your next turn, but you can Sustain it up to 1 minute.

 When you conjure the sphere and the first time you Sustain the impulse on subsequent rounds, you can either have it Fly up to 20 feet or deal 1d12 electricity damage to an adjacent creature with a basic Reflex save against your class DC.

---
**Level (+3)** The damage dealt by the sphere increases by 1d12.

*Source: Rage of Elements p. 31*
