---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-equipment
tags:
- compendium/src/pf2e/rage-of-elements
- trait/acid
- trait/consumable
- trait/talisman
- trait/uncommon
- trait/water
- equipment/level/8
aliases: ["Brine Dragon Scale"]
aon_id: "equipment-2625"
source: "Rage of Elements"
---

# Brine Dragon Scale  *Item 8*

`Acid`  `Consumable`  `Talisman`  `Uncommon`  `Water`

- **Price**: 100 gp
- **Usage**: affixed to a melee weapon
- **Activate**: [<](books/player-core/chapter-8-playing-the-game/actions#Actions "Reaction") ([[srd/pf2e/compendium/rules-elements/traits/player-core/concentrate|concentrate]])
- **Trigger**: A creature in reach of the weapon takes an action with the [[srd/pf2e/compendium/rules-elements/traits/player-core/concentrate|concentrate]] trait.

Brine dragons are known to distract their foes at just the right moment, and this blue-green scale appears to have come from one of these creatures. When you activate the scale, it cracks open and releases a spray of caustic saltwater at the triggering creature. The creature takes 2d8 acid damage with a DC 24 basic Reflex save. If the creature critically fails at its save, its concentration is broken—the triggering action is disrupted.

*Source: Rage of Elements p. 176*
