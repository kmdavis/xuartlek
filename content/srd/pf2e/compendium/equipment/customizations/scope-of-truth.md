---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-equipment
tags:
- compendium/src/pf2e/guns-gears-remastered
- trait/magical
- equipment/level/13
aliases: ["Scope of Truth"]
aon_id: "equipment-1216"
source: "Guns & Gears (Remastered)"
---

# Scope of Truth  *Item 13*

`Magical`

- **PFS Note**: Characters with access to firearms gain access to accessories that can be used with those weapons. Replace the word "transmutation" in the scope of truth with the word "polymorph."
- **Price**: 3,000 gp
- **Usage**: attached to firearm or crossbow (scope)
- **Bulk**: L

The _scope of truth_ is a bit bigger than most to accommodate larger lenses, which have been magically prepared with tiny shards from crystal balls to reveal the truth. The scope grants you a +2 item bonus to Perception checks made to Seek in areas you can see through the scope.

**Activate** [>>](books/player-core/chapter-8-playing-the-game/actions#Actions "Two-Action") [[srd/pf2e/compendium/rules-elements/traits/player-core/manipulate|manipulate]] **Frequency** once per day; **Effect** For the next 10 minutes, you can see things through the scope as they actually are. The GM rolls a [[srd/pf2e/compendium/rules-elements/traits/player-core/secret|secret]] counteract check with a +20 counteract modifier and a counteract level of 7 against any [[srd/pf2e/compendium/rules-elements/traits/player-core/illusion|illusion]] or transmutation in the area, but only for the purpose of determining whether you see through it, not to end the spell or effect. For instance, if the check succeeds against a [[srd/pf2e/compendium/rules-elements/traits/player-core/polymorph|polymorph]] spell, you can see the creature's true form, but you don't end the spell.

*Source: Guns & Gears (Remastered) p. 182*
