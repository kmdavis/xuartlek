---
cssclasses: pf2e,pf2e-equipment
tags:
- compendium/src/pf2e/player-core-2
- trait/consumable
- trait/mechanical
- trait/snare
- trait/trap
- equipment/level/14
aliases: ["Engulfing Snare"]
aon_url: "https://2e.aonprd.com/Equipment.aspx?ID=3369"
source: "Player Core 2"
---

# Engulfing Snare  *Item 14*

`Consumable`  `Mechanical`  `Snare`  `Trap`

- **Price**: 900 gp

You arrange a spiky cage of bones, particularly tough vegetation, or other material to spring up when disturbed. The snare deals 10d8 piercing damage to the first creature to enter this square; that creature must attempt a DC 34 Reflex save.

**Critical Success** The creature is unaffected.

**Success** The creature is knocked aside by the snare's deployment, takes half damage, and is [[srd/pf2e/compendium/rules-elements/conditions#Off-Guard|off-guard]] until the end of its next turn.

**Failure** The creature is captured by the cage, taking full damage and falling [[srd/pf2e/compendium/rules-elements/conditions#Prone|prone]]. It's [[srd/pf2e/compendium/rules-elements/conditions#Immobilized|immobilized]] while it remains within the cage. It can get free by [[srd/pf2e/compendium/rules-elements/actions/player-core#Escape|Escaping]] (DC 34) or by destroying the cage (AC 30, Fort +18, Ref +24, Hardness 5, HP 30, object immunities).

**Critical Failure** As failure, but the creature takes double damage.

*Source: Player Core 2 p. 298*
