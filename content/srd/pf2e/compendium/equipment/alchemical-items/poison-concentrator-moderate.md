---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-equipment
tags:
- compendium/src/pf2e/treasure-vault-remastered
- trait/alchemical
- equipment/level/10
aliases: ["Poison Concentrator (Moderate)"]
aon_id: "equipment-1981-1778"
source: "Treasure Vault (Remastered)"
---

# Poison Concentrator (Moderate)  *Item 5+*

`Alchemical`

- **Usage**: held in 2 hands
- **Bulk**: L

This compression apparatus can reduce two poisons into a more concentrated dose. As a 10-minute activity that has the [[srd/pf2e/compendium/rules-elements/traits/player-core/manipulate|manipulate]] trait, you can use a poison concentrator to combine two doses of the same [[srd/pf2e/compendium/rules-elements/traits/player-core-2/infused|infused]] alchemical poison, creating a single concentrated poison. The concentrated potion has a +1 item bonus to its DC, and its level is increased by 1.

### Poison Concentrator (Lesser)

**Price** 160 gp

**Bulk** L

---

The concentrator combines poisons of level 4 or lower.

### Poison Concentrator (Moderate)

**Price** 960 gp

**Bulk** L

---

The concentrator combines poisons of level 9 or lower.

### Poison Concentrator (Greater)

**Price** 5,900 gp

**Bulk** L

---

The concentrator combines poisons of level 14 or lower.

### Poison Concentrator (Major)

**Price** 31,000 gp

**Bulk** L

---

The concentrator combines poisons of level 18 or lower.

*Source: Treasure Vault (Remastered) p. 64*
