---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-equipment
tags:
- compendium/src/pf2e/howl-of-the-wild
- trait/invested
- trait/magical
- trait/uncommon
- equipment/level/12
aliases: ["Arctic Worm Chitin Shield"]
aon_id: "equipment-3199"
source: "Howl of the Wild"
---

# Arctic Worm Chitin Shield  *Item 12*

`Invested`  `Magical`  `Uncommon`

- **Price**: 1,800 gp
- **Usage**: held in 1 hand
- **Bulk**: 4
- **Base Shield**: Tower Shield

This tower shield is crafted from interlocking sections of an arctic cave worm’s chitin that are rimed with constant frost. It has Hardness 13, HP 104, and BT 52.

**Activate—Misty Chill** [>](books/player-core/chapter-8-playing-the-game/actions#Actions "Single Action") (cold, concentrate, manipulate) **Effect** You Raise the Shield and an icy mist begins to flow off it. Until the beginning of your next turn, you gain resistance 5 to cold and any creature that ends its turn adjacent to you takes 3d6 cold damage (DC 29 basic Reflex save).

**Craft Requirements** The initial raw materials must include chitin from an arctic cave worm.

*Source: Howl of the Wild p. 100*
