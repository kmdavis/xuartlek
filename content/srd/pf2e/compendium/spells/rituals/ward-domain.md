---
obsidianUIMode: preview
cssclasses: pf2e,pf2e-ritual
tags:
- compendium/src/pf2e/player-core-2
- trait/uncommon
- ritual/level/6
aliases: ["Ward Domain"]
aon_id: "ritual-156"
source: "Player Core 2"
---

# Ward Domain  *Ritual 6*

`Uncommon`

- **Cast**: 1 day
- **Cost**: rare incenses, oils, and powdered silver, worth 150 gp total
- **Secondary Casters**: 3
- **Primary Check**: [[srd/pf2e/compendium/rules-elements/skills/arcana|Arcana]], [[srd/pf2e/compendium/rules-elements/skills/nature|Nature]], or [[srd/pf2e/compendium/rules-elements/skills/occultism|Occultism]] (master)
- **Secondary Checks**: Lore (any), Survival , Thievery
- **Area**: 100 feet × 100 feet, up to 50 feet high
- **Duration**: 1 week

This ritual has long been used to guard the private sanctums of powerful rulers, spellcasters, and other figures of import. You and the other casters spend the casting time burning incense, anointing doorframes, and drawing lines of powered silver across entryways. The ritual creates the following magical effects within the area; these effects are heightened to the rank of _ward domain_ and remain throughout the duration.

All gates, doors, windows, and similar apertures in the area (if any) are locked, with the effects of [[srd/pf2e/compendium/spells/rank-1/lock|_lock_]]. In addition, you can obscure up to six doors, doorways, or similar entrances within the area with the effects of [[srd/pf2e/compendium/spells/rank-1/illusory-object|_illusory object_]] to appear as plain walls. [[srd/pf2e/compendium/rules-elements/traits/player-core/scrying|Scrying]] spells can’t perceive any stimuli from the area, and _ward domain_ attempts to counteract [[srd/pf2e/compendium/rules-elements/traits/player-core/teleportation|teleportation]] effects into or out of the area, including attempts to [[srd/pf2e/compendium/rules-elements/traits/player-core/summon|summon]] creatures into the area, using a modifier equal to the ritual’s save DC – 10.

A successful [[srd/pf2e/compendium/spells/rank-2/dispel-magic|_dispel magic_]] used on a specific effect removes only that effect (such as the _lock_ effect on one window). A successful [[srd/pf2e/compendium/spells/rank-9/detonate-magic|_detonate magic_]] ends the entire ritual.

**Critical Success** You create the effects described above, and you protect an area twice as large.

**Success** You create the effects described above.

**Failure** The ritual has no effect.

**Critical Failure** The area becomes trapped and hostile to you and your allies in a way you didn’t anticipate.

---
**Heightened (+1)** The ward covers an additional area 100 feet × 100 feet, up to 50 feet high, contiguous with the original area.

*Source: Player Core 2 p. 271*
