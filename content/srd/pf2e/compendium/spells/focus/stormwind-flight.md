---
cssclasses: pf2e,pf2e-spell
tags:
- compendium/src/pf2e/player-core
- trait/air
- trait/concentrate
- trait/druid
- trait/focus
- trait/manipulate
- trait/uncommon
- spell/level/4
aliases: ["Stormwind Flight"]
aon_url: "https://2e.aonprd.com/Spells.aspx?ID=1859"
source: "Player Core"
socialImage: og-image.png
---

# Stormwind Flight  *Focus 4*  [>>](books/player-core/chapter-8-playing-the-game/actions#Actions "Two-Action")

`Air`  `Concentrate`  `Druid`  `Focus`  `Manipulate`  `Uncommon`

- **Duration**: 1 minute

Powerful winds carry you through the air, giving you a fly Speed equal to your Speed. When this spell's duration would end, if you're still flying, you float to the ground, as [[srd/pf2e/compendium/spells/rank-1/gentle-landing|_gentle landing_]].

---
**Heightened (6th)** When you fly using _stormwind flight_, you ignore difficult terrain from wind.

*Source: Player Core p. 382*
