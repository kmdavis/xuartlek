---
cssclasses: pf2e,pf2e-spell
tags:
- compendium/src/pf2e/player-core-2
- trait/air
- trait/concentrate
- trait/focus
- trait/manipulate
- trait/oracle
- trait/sonic
- trait/uncommon
- spell/level/3
aliases: ["Thunderburst"]
aon_url: "https://2e.aonprd.com/Spells.aspx?ID=2088"
source: "Player Core 2"
---

# Thunderburst  *Focus 3*  [>>](books/player-core/chapter-8-playing-the-game/actions#Actions "Two-Action")

`Air`  `Concentrate`  `Focus`  `Manipulate`  `Oracle`  `Sonic`  `Uncommon`

- **Mystery**: [[srd/pf2e/compendium/character/mysteries#Tempest|Tempest]]
- **Range**: 100 feet
- **Area**: 20-foot burst
- **Defense**: Fortitude

You create a powerful blast of air and a loud peal of thunder, dealing 2d6 bludgeoning damage and 2d6 sonic damage. Each creature in the area must attempt a Fortitude save.

**Critical Success** The creature is unaffected.

**Success** The creature takes half damage.

**Failure** The creature takes full damage and is [[srd/pf2e/compendium/rules-elements/conditions#Deafened|deafened]] for 1 minute.

**Critical Failure** The creature takes double damage and is deafened for 1 hour.

---
**Heightened (+2)** Each type of damage increases by 2d6, and the area increases by 5 feet.

*Source: Player Core 2 p. 261*
