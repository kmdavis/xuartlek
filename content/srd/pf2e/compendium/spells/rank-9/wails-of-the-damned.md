---
cssclasses: pf2e,pf2e-spell
tags:
- compendium/src/pf2e/player-core
- trait/auditory
- trait/concentrate
- trait/death
- trait/manipulate
- trait/void
- trait/negative
- spell/level/9
aliases: ["Wails of the Damned"]
aon_url: "https://2e.aonprd.com/Spells.aspx?ID=1747"
source: "Player Core"
socialImage: og-image.png
---

# Wails of the Damned  *Spell 9*  [>>](books/player-core/chapter-8-playing-the-game/actions#Actions "Two-Action")

`Auditory`  `Concentrate`  `Death`  `Manipulate`  `Void`  `Negative`

- **Traditions**: Divine, Occult
- **Bloodline**: [[srd/pf2e/compendium/character/bloodlines#Undead|Undead]]
- **Area**: 40-foot emanation
- **Defense**: Fortitude

You howl a lament of damned souls. Each living enemy in the area takes 8d10 void damage depending on its Fortitude save.

**Critical Success** The creature is unaffected.

**Success** The creature takes full damage.

**Failure** The creature takes full damage and is drained 1d4.

**Critical Failure** The creature takes double damage and is drained 4.

*Source: Player Core p. 367*
