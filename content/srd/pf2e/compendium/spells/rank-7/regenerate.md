---
cssclasses: pf2e,pf2e-spell
tags:
- compendium/src/pf2e/player-core
- trait/concentrate
- trait/healing
- trait/manipulate
- trait/vitality
- trait/positive
- spell/level/7
aliases: ["Regenerate"]
aon_url: "https://2e.aonprd.com/Spells.aspx?ID=1648"
source: "Player Core"
---

# Regenerate  *Spell 7*  [>>](books/player-core/chapter-8-playing-the-game/actions#Actions "Two-Action")

`Concentrate`  `Healing`  `Manipulate`  `Vitality`  `Positive`

- **Traditions**: Divine, Primal
- **Range**: touch
- **Target**: 1 willing living creature
- **Duration**: 1 minute

An infusion of vital energy grants a creature continuous healing. The target temporarily gains regeneration 15, which restores 15 Hit Points to it at the start of each of its turns. While it has regeneration, the target can't die from taking Hit Point damage and its [[srd/pf2e/compendium/rules-elements/conditions#Dying|dying]] condition can't increase to a value that would kill it (this stops most creatures' dying condition at dying 3), though if its [[srd/pf2e/compendium/rules-elements/conditions#Wounded|wounded]] value becomes 4 or higher, it stays [[srd/pf2e/compendium/rules-elements/conditions#Unconscious|unconscious]] until its wounds are treated. If the target takes acid or fire damage, its regeneration deactivates until after the end of its next turn.

 Each time the creature regains Hit Points from regeneration, it also regrows one damaged or ruined organ (if any). During the spell's duration, the creature can also reattach severed body parts by spending an Interact action to hold the body part to the area it was severed from.

---
**Heightened (9th)** The regeneration increases to 20.

*Source: Player Core p. 352*
