---
title: "How to Use this Book"
aliases: ["How to Use this Book"]
socialImage: og-image.png
cssclasses:
  - pf2e
  - pf2e-book
tags:
  - srd/pf2e/guns-gears-remastered
source: "Guns & Gears (Remastered)"
aon_url: "https://2e.aonprd.com/Rules.aspx?ID=1636"
citation: "Guns & Gears (Remastered) pg. 5"
---

# How to Use this Book

<sup>G&G p. 5</sup>

The options here expand upon the rules contained in the *Core Rulebook*, and that book is required to use *Guns & Gears*. Some rules indicate other material found in this book with page references. Other products referencing the rules in this book might do so using the superscript citation “G&G.”

## Access Entries

Most of the rules content in this book is [uncommon](https://2e.aonprd.com/Traits.aspx?ID=159) or [rare](https://2e.aonprd.com/Traits.aspx?ID=137) because such technology is unusual in Golarion. However, sometimes a stat block for an uncommon rules element includes an Access entry that lists specific criteria. A character who meets the criteria listed in the Access entry, such as hailing from a particular locale, gains access to the rules element. This book often groups access requirements by section. For example, the introduction to the Gears Characters chapter (page 12) details the areas of Golarion that grant access to the [[srd/pf2e/compendium/character/classes/Inventor|inventor]] class and its related uncommon weapons and gadgetry, so the areas detailed there as granting access serve the role of access requirements for most of the section (not including rare items and certain other elements called out as needing additional approval from the GM, such as vehicles).

As always, the GM has the final say on who can access uncommon or rarer options or whether to allow specific options in the game at all. They might decide that a character's upbringing or abilities make sense for a specific rules option and grant them access even if the character doesn't automatically qualify, or they might decide those same options can't be taken by anyone.
